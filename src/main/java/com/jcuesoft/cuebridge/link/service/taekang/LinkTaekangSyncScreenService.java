...
public class LinkTaekangSyncScreenService {
    ...
    public LinkTaekangSyncScreenService(){
        System.out.println("LinkTaekangSyncScreenService init");
        callback = "";
    }
    ...
    public String excuteBiz(EntityManager em, Map<String, String> param) throws Exception {
        ...
        try
        {            
            if(param.get("status").equals("승인") || param.get("status").equals("입금")){
                sql = "SELECT TO_CHAR(SYSDATE, 'YYYYMMDDHH24MISS') AS DM FROM DUAL";
                dm_cooperate = (String) em.createNativeQuery(sql).getSingleResult();
            }
            if(ModuleCommon.isNumeric(param.get("money2"), false)){
                int tmpAmt = Integer.parseInt(param.get("money2").trim());
