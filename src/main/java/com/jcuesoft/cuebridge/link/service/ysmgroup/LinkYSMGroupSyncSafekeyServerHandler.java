...
public class LinkYSMGroupSyncSafekeyServerHandler extends SimpleChannelInboundHandler<Object> {

    private HttpRequest request;
    static UtilProperties util;
    static ModuleCommon mc;
    static LinkYSMGroupSyncSafekeyService service;
    String uri;
    String domain;
    String hostName;
    EntityManager em;
    ...
    public LinkYSMGroupSyncSafekeyServerHandler() throws Exception{
        System.out.println("LinkYSMGroupServerHandler init");
        callback = "Success";
        util = new UtilProperties();
        mc = new ModuleCommon();
        service = new LinkYSMGroupSyncSafekeyService();
    ...
    protected void channelRead0(ChannelHandlerContext ctx, Object msg) throws Exception {
        ...
        if (msg instanceof HttpContent) {
            ...
            if (content.isReadable()) {
                ...
                buf.append("\r\n");
                appendDecoderResult(buf, request);
                paramMap = ModuleCommon.convertStrToMap(content.toString(CharsetUtil.UTF_8));
                System.out.println("paramMap:"+paramMap);
                em = UtilJPA.getentityManagerFactory(domain).createEntityManager();   

                if(uri.matches("(.*)syncsafekey(.*)")){
                    /*==================================================================================
                     * syncsafekey service post method start
                    ==================================================================================*/
                    callback = checkValidParamSafekey(paramMap);
                    if(!callback.substring(0,1).equals("F")){                        
                        //LinkYSMGroupService service = new LinkYSMGroupService();
                        callback = service.excuteBizSyncSafekey(em, paramMap);
                    }
                    /*==================================================================================
                     * syncsafekey service post method end
                    ==================================================================================*/                    
                }else{
    ...
    private String checkValidParamSafekey(Map<String, String> param) {
        System.out.println("checkValidParamSafekey init");
        if(param.get("no_key") == null || param.get("no_key").equals("")){
            callback = "Fail: no_key parameter empty!";
            System.err.println(callback);
            buf.append(callback);
            buf.append("\r\n");
        }
        if(param.get("ds_nice_safekey") == null || param.get("ds_nice_safekey").equals("")){
            callback = "Fail: ds_nice_safekey parameter empty!";
            System.err.println(callback);
            buf.append(callback);
            buf.append("\r\n");
        }
        if(param.get("yn_surety") == null || param.get("yn_surety").equals("")){
            callback = "Fail: yn_surety parameter empty!";
            System.err.println(callback);
            buf.append(callback);
            buf.append("\r\n");
        }        
